...
class WireframeViewer(wf.WireframeGroup):
            ...
            if self.displayFaces:
                for (face, colour) in wireframe.sortedFaces():
                    ...
                    if towards_us > 0:
                        ...
                        cdiff = (self.light_color*colour)*np.dot(normal,self.light_vector)
                        r = (2*np.dot(self.light_vector,normal))*normal-self.light_vector

                        cspec = (self.light_color*colour)*(max(np.dot(self.view_vector,r),0))**mgls
                        ...
                        light_total = ambient + (cdiff*percentDiff) + (cspec*percentSpec)
    ...
    def keyEvent(self, key):
        ...
        if key == pygame.K_a:
            ...
            self.light_vector = np.matmul(wf.rotateYMatrix(math.pi/20),(np.append(self.light_vector,[1],axis = 0)))
            self.light_vector = self.light_vector[0:3]
            self.light_vector = self.light_vector/np.sqrt(np.sum(self.light_vector**2))
        if key == pygame.K_d:
            print("d is pressed")
        ...
        if key == pygame.K_w:
            ...
            self.light_vector = np.matmul(wf.rotateXMatrix(-math.pi/20),(np.append(self.light_vector,[1],axis = 0)))
            self.light_vector = self.light_vector[0:3]
            self.light_vector = self.light_vector/np.sqrt(np.sum(self.light_vector**2))
        if key == pygame.K_s:
            print("s is pressed")
        ...
        if key == pygame.K_q:
            ...
            self.light_vector = np.matmul(wf.rotateZMatrix(-math.pi/20),(np.append(self.light_vector,[1],axis = 0)))
            self.light_vector = self.light_vector[0:3]
            self.light_vector = self.light_vector/np.sqrt(np.sum(self.light_vector**2))
        if key == pygame.K_e:
            print("e is pressed")
